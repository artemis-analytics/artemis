...
"""
Book classes that work as dictionaries. Concrete classes to store, access and manage
histograms, tdigests and tool objects.
"""
...
from artemis.io.ioutils import write_many as physt_write_many
from artemis.io.ioutils import read as physt_read
from artemis_format.pymodels.histogram_pb2 import HistogramCollection
...
class ArtemisBook(BaseBook):
    ...
    def compatible(self, other):
        return set(self._iter_keys()) == set(other._iter_keys()) and all(
            self[n].has_same_bins(other[n]) for n in self.keys()
        )
        ...
        for n, x in self:
            ...
            if timer is None:
                ...
                try:
                    ...
                    self.__logger.warning("%s fails rebook, use original bins", n)
                    bins = x.binning
    ...
    def _from_message(self, msg):
        ...
        content = collections.OrderedDict(
            (n, physt_read(v)) for n, v in msg.histograms.items()
        )
...
class TDigestBook(BaseBook):
    ...
    def _digest_from_protobuf(self, protobuf):
        ...
        if not isinstance(protobuf, TDigest_instance):
            raise TypeError(
                "Error: tried to decode a " "non protobuf object into a TDigest"
            )
    ...
    def _from_message(self, msg):
        ...
        content = collections.OrderedDict(
            (n, self._digest_from_protobuf(v)) for n, v in msg.digest_map.items()
        )
        return self.__class__.load_from_dicts(content)
