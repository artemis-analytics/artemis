...
class XlsTool(ToolBase):
    ...
    def read_excel(self, location):
        ...
        is_populated = False # One workbook should only contain one dataset schema
        for sheets in sheet_list:
            if sheets == 'Dataset Info':
                if is_populated:
                    self.__logger.error("Workbook contains more than 1 dataset")
                    raise Exception
    ...
    def store_dataset(self, ds):
        ...
        '''
        pd.set_option('mode.chained_assignment', None) # Turn off warning of chained assignment
        ...
        for row in ds.iterrows():
            if (row[1][1] in ['Dataset Name','Description','Program Element','Has Sensitive Statistical Info? [y/n]','Has Personal Identifiers? [y/n]',
                             'Has Other Supporting Documentation? [y/n]','List of Expected Medium','List of Usage','Permission','Provider',
                             'Provider Type [External/Internal/Custodian]','Reference Period','Granularity Type','State','Data Asset Category',
                             'Retention Description','Retention Period','Retention Trigger Date [dd-mm-yyyy]','Retention Trigger','Retention Type',
                             'Reception Frequency','Acquisition Stage','Acquisition Cost [decimal value]','Quality Evaluation Done on Input? [y/n]',
                             'Channel Type','List of Statcan Act Sections','Channel Detail','Data Usage Type','Data Acquisition Type']
                and row[1][0] in ['Data Holding', 'Data Asset', 'Data Retention', 'Data Holding Detail', 'Provision Agreement']):
                pass
            else:
                self.__logger.error(str(row[1][1]) + " is not part of Dataset schema, please follow the template")
                raise Exception
        
        try:
            data_holding.name = str(self.mmap(ds, 'Dataset Name'))
            data_holding.description = str(ds.loc[(ds[1] == 'Description') & (ds[0] == 'Data Holding'), 2].item())
            data_holding.program_element = str(self.mmap(ds, 'Program Element'))
            data_holding.sensitive_statistical_info = self.to_bool(self.mmap(ds, 'Has Sensitive Statistical Info? [y/n]'))
            data_holding.has_personal_identifiers = self.to_bool(self.mmap(ds, 'Has Personal Identifiers? [y/n]'))
            data_holding.has_other_supporting_documentation = self.to_bool(self.mmap(ds, 'Has Other Supporting Documentation? [y/n]'))
            data_holding.expected_medium.extend(list(ds.loc[ds[1] == 'List of Expected Medium', 2:].dropna(how='all', axis=1).values.flatten()))
            data_holding.usage.extend(list(ds.loc[ds[1] == 'List of Usage', 2:].dropna(how='all', axis=1).values.flatten()))
            data_holding.permission = str(self.mmap(ds, 'Permission'))
            data_holding.provider = str(self.mmap(ds, 'Provider'))
            temp = str(self.mmap(ds, 'Provider Type [External/Internal/Custodian]')).lower()
            if temp == 'external':
        ...
        try:
            data_asset.description = str(ds.loc[(ds[1] == 'Description') & (ds[0] == 'Data Asset'), 2].item())
        ...
        try:
            ...
            dr.retention_trigger_date = str(self.mmap(ds, 'Retention Trigger Date [dd-mm-yyyy]'))
        ...
        try:
            ...
            dh_detail.acquisition_cost = float(self.mmap(ds, 'Acquisition Cost [decimal value]'))
            dh_detail.quality_evaluation_done_on_input = self.to_bool(self.mmap(ds, 'Quality Evaluation Done on Input? [y/n]'))
        except Exception:
            self.__logger.error("Error storing Data Holding Detail")
        
        try:
            pa.channel = str(self.mmap(ds, 'Channel Type'))
            pa.statcan_act_section.extend(list(ds.loc[ds[1] == 'List of Statcan Act Sections', 2:].dropna(how='all', axis=1).values.flatten()))
    ...
    def store_table(self, tb):
        ...
        if meta_name[0] == 'Variable Name':
            ...
            self.__logger.error("First column of the Table Schema should be Variable Name.")
            raise Exception
        ...
        for row in schm.iterrows():
            if row[1][0] in ['Table Name', 'Schema Name', 'Description']: 
                try:
                    ...
                    schema.info.aux.description = str(schm.loc[schm[0] == 'Description', 1].item())
                except:
                    self.__logger.error("Error storing Table level metadata")
                    raise Exception
            else:
                self.__logger.error("%s is not part of Table schema, please follow the template", str(row[1][1]))
                raise Exception
        
        for i in var_names.index: # All unique variable names in a series
            new = field.add() # Add repeated message
            temp = pd.DataFrame(tb.loc[tb[0] == tb.loc[i, 0]].drop_duplicates()) # Chunk rows for each field
            # Requires that users fill the first row for each new variable
            
            try:
                new.name = str(temp.loc[i , meta_name[meta_name == 'Variable Name'].index].item())
                new.info.type = str(temp.loc[i, meta_name[meta_name == 'Type'].index].item())
                new.info.length = int(temp.loc[i, meta_name[meta_name == 'Length'].index].item())
                new.info.nullable = self.to_bool(temp.loc[i, meta_name[meta_name == 'Nullable [y/n]'].index].item())
                new.info.aux.description = str(temp.loc[i, meta_name[meta_name == 'Description'].index].item())
            except:
            ...
            cs.name = str(temp.loc[i, meta_name[meta_name == 'Codeset Name'].index].item())
            cs.version = str(temp.loc[i, meta_name[meta_name == 'Codeset Version'].index].item())

            # Code value store:
            for j in temp.index: # Each row for variable
                newc = cs.codevalues.add()
                try:
                    newc.code = str(temp.loc[j, meta_name[meta_name == 'Code Value'].index].item())
                    newc.description = str(temp.loc[j, meta_name[meta_name == 'Code Description'].index].item())
                    newc.lable = str(temp.loc[j, meta_name[meta_name == 'Lable'].index].item())
                except:
                    self.__logger.error("Error storing codeset info")
                    raise Exception
                # Add additional info for codeset values
                for col in adin.iteritems(): 
                    if str(col[1][1]) == 'Additional Codeset Info' and str(col[1][6]) != "":
                        key = str(col[1][6])
                        if key == '':
                            self.__logger.error("Additional Codeset Info missing metadata name")
                            raise Exception
                        if col[1][2] == 'Description':
                            try:
                                newc.others[key].description = str(col[1][3])
                            except:
                                self.__logger.error("Error storing additional code set info description")
                                raise Exception
                        if col[1][4] == 'Type [null/int/string/bool/float]':
                            try:
                                vtype = str(col[1][5]).lower()
                            except:
                                self.__logger.error("Error storing additional code set info type")
                                raise Exception
                        else:
                            self.__logger.error("Additional Codeset Info missing type")
                            raise Exception
                        ...
                        if vtype == 'null':
                            ...
                            self.__logger.error("Additional Codeset Info type is not one of [null/int/string/bool/float]")
                            raise Exception
                ...
                if str(k[1][1]) == 'Additional Metadata Info' and str(k[1][6]) != "":
                    ...
                    if k[1][2] == 'Description':
                        try:
                            ...
                            self.__logger.error("Error storing additional metadata info description")
                            raise Exception
                    if k[1][4] == 'Type [null/int/string/bool/float]':
                        try:
                            mtype = str(k[1][5]).lower()
                        except:
                            self.__logger.error("Error storing additional metadata info type")
                            raise Exception
                    else:
                    ...
                    if mtype == 'null':
                        ...
                        self.__logger.error("Additional Metadata Info type is not one of [null/int/string/bool/float]")
                        raise Exception     
