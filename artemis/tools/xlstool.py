...
class XlsTool(ToolBase):
        ...
        for sheets in sheet_list:
            if sheets == 'Dataset Info':
                if is_populated:
                    ...
                    raise Exception
                dataset = self.store_dataset(pd.read_excel(xls, sheets,
                                                           header=None))
                is_populated = True
            else:
                tables.append(self.store_table(pd.read_excel(xls, sheets,
                                                             header=None)))
        if not is_populated:
    ...
    def store_dataset(self, ds):
        ...
        for row in ds.iterrows():
            if (row[1][1] in ['Dataset Name', 'Description',
                              'Program Element',
                              ...
                              'Retention Trigger', 'Retention Type',
                              'Reception Frequency', 'Acquisition Stage',
                              'Acquisition Cost [decimal value]',
                              'Quality Evaluation Done on Input? [y/n]',
                              'Channel Type', 'List of Statcan Act Sections',
                              'Channel Detail', 'Data Usage Type',
                              'Data Acquisition Type']
                and row[1][0] in ['Data Holding', 'Data Asset',
                                  'Data Retention', 'Data Holding Detail',
                                  'Provision Agreement']):
                pass
            else:
                self.__logger.error(str(row[1][1]) +
                                    " is not part of Dataset schema, "
                                    "please follow the template")
                raise Exception

        try:
            data_holding.name = \
                str(self.mmap(ds, 'Dataset Name'))
            data_holding.description = \
                str(ds.loc[(ds[1] == 'Description') &
                    (ds[0] == 'Data Holding'), 2].item())
            data_holding.program_element = \
                str(self.mmap(ds, 'Program Element'))
            data_holding.sensitive_statistical_info = \
                self.to_bool(self.mmap(ds,
                                       'Has Sensitive Statistical '
                                       'Info? [y/n]'))
            data_holding.has_personal_identifiers = \
                self.to_bool(self.mmap(ds, 'Has Personal Identifiers? [y/n]'))
            data_holding.has_other_supporting_documentation = \
                self.to_bool(self.mmap(ds,
                                       'Has Other Supporting '
                                       'Documentation? [y/n]'))
            data_holding.expected_medium.\
                extend(list(ds.loc[ds[1] ==
                            'List of Expected Medium', 2:].
                            dropna(how='all', axis=1).values.flatten()))
            data_holding.usage.\
                extend(list(ds.loc[ds[1] ==
                            'List of Usage', 2:].
                            dropna(how='all', axis=1).values.flatten()))
            ...
            temp = \
                str(self.mmap(ds,
                              'Provider Type '
                              '[External/Internal/Custodian]')).lower()
        ...
        try:
            data_asset.description = str(ds.loc[(ds[1] == 'Description') &
                                         (ds[0] == 'Data Asset'), 2].item())
            data_asset.reference_period = \
                str(self.mmap(ds, 'Reference Period'))
            data_asset.granularity_type = \
        ...
        try:
            ...
            dr.retention_trigger_date = \
                str(self.mmap(ds, 'Retention Trigger Date [dd-mm-yyyy]'))
            dr.retention_trigger = \
                str(self.mmap(ds, 'Retention Trigger'))
            dr.type = str(self.mmap(ds, 'Retention Type'))
        except Exception:
            self.__logger.error("Error storing Data Retention")

        try:
            dh_detail.receptionFrequency = \
                str(self.mmap(ds, 'Reception Frequency'))
            dh_detail.acquisition_stage = \
                str(self.mmap(ds, 'Acquisition Stage'))
            dh_detail.acquisition_cost = \
                float(self.mmap(ds, 'Acquisition Cost [decimal value]'))
            dh_detail.quality_evaluation_done_on_input = \
                self.to_bool(self.mmap(ds,
                                       'Quality Evaluation '
                                       'Done on Input? [y/n]'))
        except Exception:
        ...
        try:
            ...
            pa.statcan_act_section.extend(list(ds.loc[ds[1]
                                          == 'List of Statcan Act Sections',
                                                      2:].
                                               dropna(how='all', axis=1).
                                               values.flatten()))
            pa.channel_detail = \
                str(self.mmap(ds, 'Channel Detail'))
            pa.data_usage_type = \
                str(self.mmap(ds, 'Data Usage Type'))
            pa.data_acquisition_type = \
                str(self.mmap(ds, 'Data Acquisition Type'))
        except Exception:
    ...
    def store_table(self, tb):
        ...
        if meta_name[0] == 'Variable Name':
            ...
            self.__logger.error("First column of the Table Schema "
        ...
        for row in schm.iterrows():
            if row[1][0] in ['Table Name', 'Schema Name', 'Description']:
                try:
                    table.name = \
                        str(schm.loc[schm[0] == 'Table Name', 1].item())
                    schema.name = \
                        str(schm.loc[schm[0] == 'Schema Name', 1].item())
                    schema.info.aux.description = \
        ...
        for i in var_names.index:  # All unique variable names in a series
            ...
            try:
                new.name = \
                    str(temp.loc[i, meta_name[meta_name == 'Variable Name'].
                                 index].item())
                new.info.type = \
                    str(temp.loc[i, meta_name[meta_name == 'Type'].
                                 index].item())
                new.info.length = \
                    int(temp.loc[i, meta_name[meta_name == 'Length'].
                                 index].item())
                new.info.nullable = \
                    self.to_bool(temp.loc[i, meta_name[meta_name ==
                                                       'Nullable [y/n]'].
                                          index].item())
            ...
            for j in temp.index:  # Each row for variable
                ...
                try:
                    newc.code = str(temp.loc[j,
                                    meta_name[meta_name == 'Code Value'].
                                             index].item())
                    newc.description = str(temp.loc[j,
                                           meta_name[meta_name
                                                     == 'Code Description'].
                                                    index].item())
                    newc.lable = str(temp.loc[j,
                                              meta_name[meta_name
                                                        == 'Lable'].
                                              index].item())
                except Exception:
                ...
                for col in adin.iteritems():
                    if str(col[1][1]) == 'Additional Codeset Info' \
                            and str(col[1][6]) != "":
                        key = str(col[1][6])
                        if key == '':
                            self.__logger.error("Additional Codeset Info "
                        ...
                        if col[1][2] == 'Description':
                            try:
                                ...
                                self.__logger.error("Error storing additional "
                        ...
                        if col[1][4] == 'Type [null/int/string/bool/float]':
                            ...
                            self.__logger.error("Additional Codeset "
                                                "Info missing type")
                            raise Exception
            ...
            for k in adin.iteritems():
                if str(k[1][1]) == 'Additional Metadata Info' \
                        and str(k[1][6]) != "":
                    key = str(k[1][6])
                    if key == '':
                        self.__logger.error("Additional Metadata Info "
                                            "missing name")
                        raise Exception
                    if k[1][2] == 'Description':
                        try:
                            ...
                            self.__logger.error("Error storing "
                                                "additional metadata "
                                                "info description")
                            raise Exception
                    if k[1][4] == 'Type [null/int/string/bool/float]':
                        ...
                        self.__logger.error("Additional Metadata "
                                            "Info missing type")
                        raise Exception
                    info = temp.loc[i, k[0]]
    ...
    def to_bool(self, cell):
        ...
        if temp in ['y', 'n', '']:
            ...
            self.__logger.error('Boolean field contains unexpected value: '
                                + temp)
            raise Exception
