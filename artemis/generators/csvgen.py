...
class GenCsvLike:
    ...
    def generate(self):
        ...
        while i < self.nchunks:
            ...
            # Should be bytes.
            self.__logger.debug(
                "%s: type data: %s" % (self.__class__.__name__, type(getdata))
            )
        ...
        self.__logger.debug(
            "%s: Average of total: %2.1f" % (self.__class__.__name__, mysum / i)
        )
        # Same as previous.
        self.__logger.debug(
            "%s: Average of size: %2.1f" % (self.__class__.__name__, mysumsize / i)
        )
...
class GenCsvLikeArrow(GeneratorBase):
    ...
    def initialize(self):
        ...
        for _ in range(self.num_cols):
            self.types.append(
                self.pa_types[self._builtin_generator.rnd.randint(0, _tr)]
            )
    ...
    def make_random_csv(self):
        ...
        arr = self._builtin_generator.rnd.randint(
            0, 1000, size=(self.num_cols, self.num_rows)
        )
    ...
    def make_mixed_random_csv(self):
        ...
        for icol in range(self.num_cols):
            ty, data = self._builtin_generator.get_type_and_builtins(
                self.num_rows, self.types[icol]
            )
            columns[icol] = data
    ...
    def write(self):
        ...
        while True:
            ...
            ds_id = self.gate.meta.parentset_id
            id_ = self.gate.store.register_content(
                buf, fileinfo, dataset_id=ds_id, partition_key=self.name, job_id=job_id
            ).uuid
            self.gate.store.put(id_, buf)
            self._batchidx += 1
