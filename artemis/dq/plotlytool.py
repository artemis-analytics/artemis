...
class ProcessHist:
    ...
    def _get_hist_obj(histogram=None):
        ...
        if os.path.exists(address):
            ...
            logging.error(
                "Invalid Cronus Histogram Address: '%s'.",
                address
            )
            logging.info(
            ...
            for iteration, histogram in enumerate(histograms):
                ...
                for item in req_hist_names:
                    if item.lower() in ["", "all"]:
                        ...
                        logging.error(
                            "Invalid Requested Histogram Name: '%s'.",
                            item
                        )
                        logging.info(
...
class ProcessTDigest:
    ...
    def _calculate_cdf(tdigest=None, method=""):
        ...
        if method == "uniform":
            ...
            uniform_x = numpy.linspace(
                start=tdigest.percentile(0),
                stop=tdigest.percentile(100),
                num=100
            )
            uniform_cdf = []
    ...
    def _get_digest_map(tdigest=None):
        ...
        for key in book:
            ...
            digest = TDigest()
            digest.update_centroids_from_list(
                book[dataset_name].centroids_to_list()
            )
    ...
    def get_centroids(self, digest_map=None, name=""):
        ...
        logging.info(
            "Creating TDigest Data Using %s...",
            CDF_ANALYSIS_METHOD.lower()
        )
            ...
            for iteration, tdigest in enumerate(tdigests):
                ...
                for name in req_names:
                    ...
                    if all([name not in all_names, name != "all"]):
                        logging.error(
                            "Invalid Requested TDigest Name: '%s'.",
                            name
                        )
                        logging.info(
...
class MergeTraces:
    ...
    def _validate(traces=None, max_cols=0):
        ...
        if not isinstance(traces, list):
            logging.error(
                "Invalid Traces Parameter Type: '%s'.",
                type(traces)
            )
            logging.info(
        ...
        if isinstance(max_cols, int):
            ...
            logging.error(
                "Invalid Maximum Subplot Columns Variable Type: '%s'.",
                type(max_cols)
            )
            logging.warning(
    ...
    def merge(self):
        ...
        traces, max_cols = self._validate(
            traces=self.traces,
...
class BuildFigure:
    ...
    def _validate(traces=None, figure_type=""):
        ...
        if not isinstance(figure_type, str):
            logging.error(
                "Invalid Figure Type Parameter: '%s'.",
                type(figure_type)
            )
            logging.info(
    ...
    def generate_figure(self):
        ...
        if traces:
            ...
            if trace_objects:
                logging.info(
                    "Combining %s Figure(s) in Subplot...",
                    len(trace_objects)
                )
    ...
    def visualize(self, output="", show=True, check=True):
        ...
        if self._check_output(output=output, check=check):
            ...
            hist_traces = MergeTraces(
                traces=hist_traces,
                max_cols=MAX_HIST_SUBPLOT_COLUMNS
            ).merge()
            ...
            hist_traces = MergeTraces(
                ...
                max_cols=MAX_TDIGEST_SUBPLOT_COLUMNS
            ).merge()
        ...
        logging.info(
            "Time to Execute `PlotlyTool`: %s seconds.",
